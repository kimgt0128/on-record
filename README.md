...
보안 적용사항 문서 기준으로 온기록은 "LLM이 개인정보를 마스킹한다"는 구조가 아니라, 외부 LLM 호출 전에 백엔드의 Privacy Gateway가 먼저 개인정보를 가명처리하는 흐름을 전제로 설계했습니다.

```mermaid
flowchart LR
  A["Whisper 원본 전사"] --> B["Privacy Gateway"]
  B --> C["선마스킹 / 토큰화"]
  C --> D["LLM Agent 판단"]
  D --> E["정제된 전사 저장"]
```

- 외부 LLM 전송 전 이름, 주소, 전화번호 같은 개인정보를 먼저 치환
- 주민번호, 계좌번호 등 고위험 정보는 외부 LLM으로 보내지 않고 차단
- 원본 전사와 PII 매핑 정보는 짧은 TTL의 임시 저장소에만 두고 작업 후 삭제
- DB에는 정제된 전사와 원문 없는 감사 로그만 남기는 구조를 기준으로 설계
- 수신자별 보고서는 기관, 간호사, 가족, 외부 기관에 맞춰 마스킹 수준을 다르게 적용
- 보호 이벤트 영상은 암호화 저장과 접근 로그를 전제로 함

현재 프로토타입은 정적 HTML 데모이므로 위 보안 정책을 실제 백엔드로 집행하지는 않습니다. 대신 동의 확인, 마스킹 알림, 수신자별 보고서 구분, `[보안 인증 후 열람]` 같은 UI 흐름으로 처리 방식을 보여주고 있습니다.
